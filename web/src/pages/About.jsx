...
/* ============================================================
   Nestora — "About" page, maxed-out interactive.
   All vanilla (no deps). Effects:
   - canvas particle constellation that repels from the cursor
   - custom trailing cursor (dot + ring) on desktop
   - top scroll-progress bar
   - split-text letter-by-letter heading animation
   - 3D tilt cards with pointer-follow glow
   - count-up stats
   - magnetic buttons
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      particles = Array.from({ length: count }, () => ({
        ...
        r: Math.random() * 1.8 + 1,
        c: Math.random() > 0.5 ? brand : accent,
      }));
    }
...
function CursorFX() {
  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      const t = e.target;
      const interactive = t.closest && t.closest('a, button, .tilt-card, .about-stat');
    ...
    loop();
    return () => {
      document.body.classList.remove('about-cursor-on');
...
/* ---------- Split-text heading: animates letters in ---------- */
    ...
    <span ref={ref} className={`split ${shown ? 'in' : ''} ${className}`} aria-label={text}>
      {words.map((word, wi) => (
        <span key={wi} className="split-word">
          {[...word].map((ch) => {
            const d = idx++;
            return <span key={d} className="split-char" style={{ transitionDelay: `${d * 28}ms` }}>{ch}</span>;
          })}
...
function MagneticButton({ to, children, ghost }) {
  ...
  return (
    <Link ref={ref} to={to} className={`btn ${ghost ? 'btn-ghost' : 'btn-primary'} btn-lg magnetic`} onMouseMove={onMove} onMouseLeave={onLeave}>
      {children}
    </Link>
  );
}
...
export function AboutPage() {
  ...
  const steps = [
    { n: '01', t: 'Исследуйте', d: 'Откройте каталог или карту, листайте 360°-туры и задавайте вопросы ИИ.' },
    { n: '02', t: 'Сравните', d: 'ИИ-агент сопоставит варианты и честно оценит цену относительно рынка.' },
    { n: '03', t: 'Бронируйте', d: 'Заявка, диалог с продавцом и оплата — всё онлайн, за пару кликов.' },
  ];
  const values = [
  ...
  return (
    <div className="about-page">
      ...
      <section className="about-hero" ref={heroRef}>
        ...
        <Constellation />
        <div className="about-hero-spot" aria-hidden="true" />
        <div className="container about-hero-inner">
          <div className="about-eyebrow reveal-now">
            <span className="pulse-dot" /> {lang === 'ru' ? 'О Nestora' : 'About Nestora'}
          </div>
          <h1 className="about-hero-title" style={{ transform: 'translate3d(calc(var(--px,0) * 10px), calc(var(--py,0) * 10px), 0)' }}>
            <SplitText text={lang === 'ru' ? 'Недвижимость' : 'Real estate'} />
            <br />
            <em><SplitText text={lang === 'ru' ? 'видно насквозь' : 'you see through'} /></em>
          </h1>
          <Reveal delay={500}>
      ...
      <section className="container about-stats-wrap">
        <div className="about-stats">
          {[
            { to: 55, suffix: '+', ru: 'Активных объектов', en: 'Active listings' },
            ...
            <Reveal key={i} className="about-stat" delay={i * 90}>
      ...
      {/* ===== HOW IT WORKS ===== */}
      <section className="container about-section">
        <Reveal><h2 className="about-h2">{lang === 'ru' ? 'Как это работает' : 'How it works'}</h2></Reveal>
        <div className="about-steps">
          {steps.map((s, i) => (
            <Reveal key={i} className="about-step" delay={i * 110}>
              <div className="about-step-n">{s.n}</div>
              <h3>{lang === 'ru' ? s.t : EN_STEP[s.t]?.t || s.t}</h3>
              <p>{lang === 'ru' ? s.d : EN_STEP[s.t]?.d || s.d}</p>
            </Reveal>
          ))}
        </div>
      </section>
...
const EN_STEP = {
  'Исследуйте': { t: 'Explore', d: 'Open the catalog or map, browse 360° tours and ask the AI questions.' },
  'Сравните': { t: 'Compare', d: 'The AI agent matches options and honestly rates the price vs the market.' },
  'Бронируйте': { t: 'Book', d: 'Request, chat with the seller and pay — all online, in a couple of clicks.' },
};
const EN_VAL = {
